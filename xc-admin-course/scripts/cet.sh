...
v_azure_creds_name="learnf5-aws"
...
echo "Prerequisites:"
echo ""
echo "The cloud site, AWS or Azure, must be created"
echo ""
...
echo "All Azure VNET cloud sites ..."
curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/config/namespaces/system/azure_vnet_sites" | jq | grep -w "name"
}
...
curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/config/namespaces/system/aws_vpc_sites" | jq
echo "All Azure VNET cloud sites ..."
curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/config/namespaces/system/azure_vnet_sites" | jq -r .[][].name | grep -E "$1"
}
