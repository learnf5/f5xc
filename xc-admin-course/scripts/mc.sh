...
Class setup script for Multi Cloud
...
-tok                       Test token
-mccns <studentname>       MC - Create namespace
-mcckl <studentname>       MC - Create key, label
-mccvs <studentname>       MC - Create virtual site
-mcclsaws <studentname>    MC - Create cloud site - AWS
-mcappaws <studentname>    MC - Apply cloud site - AWS
-mcclsazu <studentname>    MC - Create cloud site - Azure
-mcappazu <studentname>    MC - Apply cloud site - Azure

-mcdso <studentname>       MC - Delete single student objects
...
student@pc14:~/f5xc/xc-admin-course/scripts$ cat mc.sh
#!/bin/bash
...
echo "-mccvs <studentname>       MC - Create virtual site"
...
while [ $# -gt 0 ]; do
 case "$1" in
   ...
   f_echo "Creating virtual for $2 ..."
   f_mc_create_single_student_vs() $2
   ;;
   -mcclsaws)
   if [ "$#" != 2 ]; then
    f_echo "Missing student name ... "
