...
### Points to the First Public Training F5XC console which points to the Training AWS instance
v_token="u9gTu7Yp55Pfvon+MmLXpavWV7uAYXw="
...
echo "-adcre12 <studentname>     ADMIN - Create lab 12 student objects"
echo "-adcre13 <studentname>     ADMIN - Create lab 13 student objects"
...
sleep 2
echo "Deleting API credentials...Not built yet ..."
### These are generated with random names, however both have studentX in them
### curl -s -H "Authorization: APIToken $v_token" -X DELETE "$v_url/web/namespaces/system/api_credentials" | jq
sleep 2
...
### curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/config/namespaces/$1/virtual_k8ss/$1-vk8s" | jq
### curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/config/namespaces/student7/virtual_k8ss/student7-vk8s" | jq
...
f_admin_create_single_student_objects_labs12()
{
### Create labs 12 for studentX
...
f_admin_create_single_student_objects_labs13()
{
### Create labs 13 for studentX
s_op="$1-k8s"
s_lb="$1-https-lb"
s_dom="$1.$v_dom"
...
echo "Known API credentials ..."
curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/web/namespaces/system/api_credentials" | jq -r .[][].name | grep -E 'student[1-9]{1}'
echo "Listing student API credentials ..."
curl -s -H "Authorization: APIToken $v_token" -X GET "$v_url/web/namespaces/system/api_credentials/student14-wpxlsges-wcqqaspk" | jq
sleep 1
echo "Revoking student API credentials ..."
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/web/namespaces/system/revoke/api_credentials" -d '{"name":"student14-wpxlsges-wcqqaspk"}' | jq
sleep 1
echo "Deleting API credentials ..."
### curl -s -H "Authorization: APIToken $v_token" -X DELETE "$v_url/web/namespaces/system/api_credentials/student14-wpxlsges-wcqqaspk" | jq
}
...
while [ $# -gt 0 ]; do
 case "$1" in
   ...
   f_echo "Creating ADMIN lab 12 objects for $2 ..."
   f_admin_create_single_student_objects_labs12 $2
   ;;
   -adcre13)
   if [ "$#" != 2 ]; then
    f_echo "Missing student name ... "
   ...
   f_echo "Creating ADMIN lab 12 objects for $2 ..."
   f_admin_create_single_student_objects_labs13 $2
   ;;
   -wadso)
   if [ "$#" != 2 ]; then
    f_echo "Missing student name ... "
