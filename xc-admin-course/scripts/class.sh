...
echo "-adcre7 <studentname> <Private DNS Name IPv4>  ADMIN - Create labs 7,8,10 student objects"
echo "-adcre8 <studentname>                          ADMIN - Create labs 7,8,10 student objects"
echo "-adcre10 <studentname>                         ADMIN - Create labs 7,8,10 student objects"
...
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/config/namespaces/shared/origin_pools" -d '{"metadata":{"name":"'$s_op'","namespace":"'$1'"},"spec":{"origin_servers":[{"private_name":{"dns_name":"'$s_ipdns_name'","outside_network":{},"site_locator":{"virtual_site":{"name":"shared/'$1'-vsite","namespace":"'$1'"}},"port":"22"}}]}}'
}
...
while [ $# -gt 0 ]; do
 case "$1" in
   ...
   if [ "$#" != 3 ]; then
    f_echo "Missing student private IP DNS name ... "
    exit 1
   fi
