...
### Points to the Training F5XC console which points to the Training AWS instance
### v_token="9F9igLid8gdh9ZJZNJAlLUOaxSs="
### v_url="https://training.console.ves.volterra.io/api"

### Tenant matters JuiceShop is studentX-juice.aws.learnf5.cloud or studentX-juice.dev.learnf5.cloud
...
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/config/namespaces/$1/origin_pools" -d '{"metadata":{"name":"'$1'-juice-originpool"},"spec":{"gc_spec":{},"origin_servers":[{"public_ip":{"ip":"23.22.60.254"},"labels":{}}],"port":"'$pnum'","healthcheck":[{"namespace":"student4","name":"'$1'-hc"}]}}' | jq
sleep 1
echo "Creating HTTP Load Balancer for $1 ..."
### Check whether on production or dev XC tenant
### Tenant matters JuiceShop is studentX-juice.aws.learnf5.cloud or studentX-juice.dev.learnf5.cloud
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/config/namespaces/$1/http_loadbalancers" -d '{ "metadata":{"name":"'$1'-juice-lb","namespace":"'$1'"},"spec":{"domains":["'$1'-juice.dev.learnf5.cloud"],"https_auto_cert":{"add_hsts":true,"http_redirect":true},"port":"443","default_route_pools":[{"pool":{"namespace":"'$1'","name":"'$1'-juice-originpool"},"weight":"1","priority":"1"}],"add_location":true,"enable_api_discovery":{"enable_learn_from_redirect_traffic":{},"discovered_api_settings":{"purge_duration_for_inactive_discovered_apis":2}}} }' | jq
}
