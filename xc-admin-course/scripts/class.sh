...
v_token="KTupu7Yp55Pfvon+MmLXpavWV7uAYXw="
...
echo "-adkub7 <studentname>      ADMIN - Download Kubeconfig in lab 7"
...
f_admin_create_single_student_objects_labs7kub()
{
s_vk8s="$1-vk8s"
echo "Downloading vK8s Kubeconfig file for $1 ..."
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/web/namespaces/$1/api_credentials" -d '{"name":"'$s_vk8s'","namespaces":"system","expiration_days":30,"spec":{"type":"KUBE_CONFIG","users":[],"password":null,"virtual_k8s_name":"'$s_vk8s'","virtual_k8s_namespace":"'$1'"}}'
echo ""
...
while [ $# -gt 0 ]; do
 case "$1" in
   ...
   f_echo "Downloading Kubeconfig file in lab 7 for $2 ..."
   f_admin_create_single_student_objects_labs7kub()
   ;;
   -adcre9)
   if [ "$#" != 2 ]; then
