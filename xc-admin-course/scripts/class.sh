...
### v_token="a68f9F9igLid8gdh9ZJZNJAlLUOaxSs="
...
echo "-adcre <studentname>       ADMIN - Create first labs student objects"
...
echo "Creating ADMIN labs 2 to 4 for $1 ..."
echo ""
...
curl -s -H "Authorization: APIToken $v_token" -X POST "$v_url/config/namespaces/shared/virtual_sites" -d '{"metadata":"{"name":"'$s_vsite_name'","namespace":"'$1'"},"spec":{"site_type":"CUSTOMER_EDGE","site_selector":{"expressions":{"'$s_key'","in","'$s_value'"}}}}'
}
...
while [ $# -gt 0 ]; do
 case "$1" in
   ...
   f_echo "Creating first ADMIN lab objects for $2 ..."
